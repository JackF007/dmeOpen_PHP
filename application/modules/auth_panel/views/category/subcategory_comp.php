...
<?php //echo $title; ?>
        ...
        <div class="panel-body">
            <form method="post" action="<?=AUTH_PANEL_URL . 'Category/add_static_content_comp/'?>" enctype="multipart/form-data">
                <div class="adv-table"style>
                    <span id="err" style="color:brown;display:none">Page content can not be blank.</span>
                    <textarea style="width:100%;height:200px" required="required"  id="child_content" name="child_content" placeholder="Enter content here..."><?php if($content['child_content']!='')echo $content['child_content'];?></textarea>
                </div>

                <input type="hidden" id="cat" name="sub_cat" value="<?=$id?>">
        ...
        <div class="form-group ">
				   <div class="col-lg-12" style="margin-bottom:10px">
                        ...
                        else{ echo base_url()."images/child_static_content/".$file[$i];}
                        ?>' style='width:150px;height:150px'>
                        <a href="<?=AUTH_PANEL_URL.'Category/delete_image1/'.$id.'/'.$cat.'/'.$file[$i]?>"><img style="margin: -90px;height: 25px;" src="<?=AUTH_ASSETS.'img/details_close.png'?>"></a>
...
<script>
    ...
    url: '<?='http://52.205.20.98/gyacomo/index.php/auth_panel/category/add_static_content_comp/'?>',
    ...
    init: function() {
        ...
        document.getElementById("submit-all").addEventListener("click", function(e) {
            ...
            if (dzClosure.files.length) {
                ...
                //dzClosure.processQueue(); // just submit the form
                $.ajax({
                   method:'POST',
                   dataType:'json',
                   data: { 
                       child_id:jQuery("#cat").val(),
                        child_content:$("[name='child_content']").val(),
                        add:$("[name='child_content']").val()
                       },
                   url:'<?='http://52.205.20.98/gyacomo/index.php/auth_panel/category/add_static_content_comp/'?>',
        ...
        this.on("sendingmultiple", function(data, xhr, formData) {
              formData.append("child_id", jQuery("#cat").val());
              // formData.append("sub_category_id", jQuery("#sub_cat").val());
              formData.append("child_content", $("[name='child_content']").val());
              formData.append("add", $("[name='child_content']").val());
